...
/* WebGLPlaceholder — Entry component. Renders 3D WebGL scene if supported,
   otherwise falls back to the original gradient PlaceholderImage. */

import dynamic from 'next/dynamic'
import { useWebGLCapability } from './useWebGLCapability'

// Inline gradient fallback (same as PlaceholderImage)
...
const IconMap: Record<string, React.FC<{ className?: string }>> = {
  building: Building2, rocket: Rocket, trending: TrendingUp, users: Users,
  chart: BarChart3, shield: Shield, book: BookOpen, briefcase: Briefcase,
  map: MapPin, landmark: Landmark,
}

// Dynamic import of the WebGL inner component (no SSR)
const WebGLPlaceholderInner = dynamic(
  () => import('./WebGLPlaceholderInner'),
  { ssr: false }
)
...
function GradientFallback({ theme = 'default', aspectRatio = 'aspect-video', label, className = '', overlay = true }: Props) {
  ...
  return (
    <div className={`relative ${aspectRatio} bg-gradient-to-br ${t.gradient} rounded-lg overflow-hidden ${className}`}>
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.1) 1px, transparent 1px)', backgroundSize: '30px 30px' }} />
      </div>
...
export default function WebGLPlaceholder(props: Props) {
  const hasWebGL = useWebGLCapability()

  // Hero theme always uses gradient fallback (per requirements)
  if (props.theme === 'hero' || !hasWebGL) {
    return <GradientFallback {...props} />
  }

  return <WebGLPlaceholderInner {...props} />
}
