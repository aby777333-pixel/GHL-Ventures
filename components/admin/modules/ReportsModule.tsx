    ...
    <div className="space-y-6 admin-section-enter">
      ...
      <div className="flex gap-1 p-1 bg-white/[0.03] rounded-xl border border-white/[0.06] overflow-x-auto scrollbar-hide">
        {TABS.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => handleTabClick(tab.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-medium transition-all duration-300 whitespace-nowrap ${isActive ? 'bg-brand-red/20 text-white border border-brand-red/30' : 'text-gray-300 hover:text-white hover:bg-white/[0.08]'}`}>
