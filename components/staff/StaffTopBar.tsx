...
import { Search, Bell, Menu, ChevronRight, Info, AlertCircle, CheckCircle, AlertTriangle, X } from 'lucide-react'
import type { StaffModule, AgentStatus } from '@/lib/staff/staffTypes'
import { STAFF_MODULE_LABELS, STAFF_SIDEBAR_ITEMS } from '@/lib/staff/staffConstants'
...
export default function StaffTopBar({
  ...
  // Search across staff nav items + sub-items.
  const searchResults = (() => {
    const q = searchQuery.trim().toLowerCase()
    if (!q) return [] as { id: string; label: string; href: string; parent?: string }[]
    const out: { id: string; label: string; href: string; parent?: string }[] = []
    for (const item of STAFF_SIDEBAR_ITEMS) {
      if (item.label.toLowerCase().includes(q)) out.push({ id: item.id, label: item.label, href: item.href })
      if (item.subItems) {
        for (const sub of item.subItems) {
          if (sub.label.toLowerCase().includes(q) || sub.id.toLowerCase().includes(q)) {
            out.push({ id: sub.id, label: sub.label, href: sub.href, parent: item.label })
          }
        }
      }
    }
  ...
  // Subscribe to the notifications table for rows targeted at this staff user
  // (assigned tasks, ticket updates, HR approvals). Without this, only chat /
  // leads / contact events would ring — everything else stays silent until
  // a page reload.
  useEffect(() => {
    let mounted = true
    ...
    getStaffSession().then(s => {
      ...
      unsub = onClientNotification(uid, (payload: any) => {
        ...
        sendStaffBrowserNotification(String(n.title || 'Notification'), String(n.message || ''))
      })
    })
        ...
        <div className="hidden md:flex items-center flex-1 max-w-sm mx-4">
          <div className="relative w-full" ref={searchRef}>
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-teal-500/30 transition-colors"
            />
            {searchQuery && (
              ...
              <div className="absolute top-full left-0 right-0 mt-1 rounded-xl bg-[#111]/95 border border-white/[0.08] shadow-2xl overflow-hidden z-50" style={{ backdropFilter: 'blur(20px)' }}>
                {searchResults.map(r => (
                  <button
                    ...
                    onMouseDown={(e) => {
                      e.preventDefault()
                      // StaffClient's navigate expects a path relative to /staff
                      // (e.g. 'me/profile'), not the full '/staff/me/profile' href.
                      const rel = r.href.replace(/^\/staff\/?/, '') || 'home'
                      navigate(rel)
                      setSearchQuery('')
