...
export async function createSupportTicket(ticket: Record<string, any>) {
  ...
  // Build clean row with only valid ticket columns
  const row: Record<string, any> = {
    ...
    source: ticket.source || 'dashboard',
...
export async function fetchAssignedRM(clientId?: string): Promise<{ name: string; designation: string; department: string } | null> {
  ...
  try {
    ...
    // (clients can't read other users' profiles due to RLS)
    const { data, error } = await sb.rpc('get_assigned_rm_info', { p_client_id: clientId })
    ...
    // Fallback: try direct queries (works for admin/staff users)
